...
function updateView() {
    if (visionActual === 'table') {
        document.getElementById('contenido').innerHTML = generateTable(ropaHombre);
    } else if (visionActual === 'list') {
        document.getElementById('contenido').innerHTML = generateList(ropaHombre);
    }
}
...
function generateTable(ropaHombre) {
    return `
        <table>
            <thead>
                <tr>
...
function showProductDetails(productId) {
    const producto = ropaHombre.find(product => product.id === productId);
    if (producto) {
